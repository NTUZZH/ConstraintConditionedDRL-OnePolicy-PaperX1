...
def available_mch_list_for_job(chosen_job, env):
    """Machines the chosen job can actually START on now.

    Reads env.dynamic_pair_mask -- the same legality mask the learned policy's
    actor is given: static incompatibility OR pair_free_time >
    next_schedule_time. In the constraint family, pair_free_time is the setup-
    and outage-adjusted start, so a machine that is mid-outage, or that would
    need a changeover it cannot fit, is masked out. Because every dispatching
    rule routes through this mask, rules and policy choose from exactly the
    same action set. The raw mch_free_time clock is not a legal-start test
    under SETUP/WINDOW: it ignores changeovers and the outage calendar.

    Ties are broken by earliest legal start.
    """
    legal = np.where(~env.dynamic_pair_mask[0, chosen_job])[0]
    if legal.size == 0:
        # no pair can start this instant; fall back to static compatibility so
        # the rule still emits an action and the episode can advance
        legal = np.where(~env.candidate_process_relation[0, chosen_job])[0]
        start = env.mch_free_time[0][legal]
    else:
        start = env.pair_free_time[0, chosen_job][legal]
    return legal[min_element_index(start)]
...
def heuristic_select_action(method, env):
    ...
    if method == 'FIFO':
        ...
        # earliest machine that can LEGALLY start it (same mask the policy gets)
