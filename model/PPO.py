...
class Memory:
    def __init__(self, gamma, gae_lambda):
        ...
        # (PPVC, Adaptation 1) static type indices, entries None outside PPVC mode
        self.op_type_seq = []  # [N, tensor[sz_b, N] or None]
        self.mch_type_seq = []  # [N, tensor[sz_b, M] or None]
        # (this paper) constraint token g, entries None outside FAMILY mode
        self.token_seq = []  # [N, tensor[sz_b, token_dim] or None]
    ...
    def clear_state(self):
        ...
        del self.op_type_seq[:]
        del self.mch_type_seq[:]
        del self.token_seq[:]
    ...
    def push(self, state):
        ...
        self.op_type_seq.append(state.op_type_tensor)
        self.mch_type_seq.append(state.mch_type_tensor)
        self.token_seq.append(getattr(state, 'token_tensor', None))
    ...
    def transpose_data(self):
        ...
        # (PPVC, Adaptation 1) type indices flattened the same way as fea_j
        # so minibatch slicing stays aligned; None outside PPVC mode
        if len(self.op_type_seq) > 0 and self.op_type_seq[0] is not None:
            t_op_type_seq = torch.stack(self.op_type_seq, dim=0).transpose(0, 1).flatten(0, 1)
            t_mch_type_seq = torch.stack(self.mch_type_seq, dim=0).transpose(0, 1).flatten(0, 1)
        else:
            t_op_type_seq = None
            t_mch_type_seq = None

        # (this paper) token flattened the same way; None outside FAMILY mode
        if len(self.token_seq) > 0 and self.token_seq[0] is not None:
            t_token_seq = torch.stack(self.token_seq, dim=0).transpose(0, 1).flatten(0, 1)
        else:
            t_token_seq = None
        ...
        return t_Fea_j_seq, t_op_mask_seq, t_Fea_m_seq, t_mch_mask_seq, t_dynamicMask_seq, \
               ...
               t_op_type_seq, t_mch_type_seq, t_token_seq
...
class PPO:
    ...
    def update(self, memory):
        ...
        # t_data[12] = logprobs (t_data[-1] is now the optional mch_type,
        # which is None outside PPVC mode)
        full_batch_size = len(t_data[12])
        ...
        for _ in range(self.k_epochs):
            ...
            for i in range(int(num_batch)):
                ...
                pis, vals = self.policy(fea_j=t_data[0][start_idx:end_idx],
                                        ...
                                        op_type=t_data[13][start_idx:end_idx]
                                        if t_data[13] is not None else None,
                                        mch_type=t_data[14][start_idx:end_idx]
                                        if t_data[14] is not None else None,
                                        token=t_data[15][start_idx:end_idx]
                                        if t_data[15] is not None else None)
